...
import { updatePayGroup } from '@/lib/actions/pay-groups'
...
const schema = z
  ...
  .superRefine((value, ctx) => {
    ...
    if (value.pay_frequency === 'BI_WEEKLY') {
      ...
      const w = Number(value.pay_day_of_week ?? '')
      if (Number.isInteger(w) && w >= 1 && w <= 7 && weekdayMon1Sun7(anchor) !== w) {
        ctx.addIssue({
          code: z.ZodIssueCode.custom,
...
type EditPayGroupModalProps = {
  payGroup: PayGroupRow
  open: boolean
  onOpenChange: (open: boolean) => void
  onSuccess: () => void
}

function EditPayGroupFormBody({
  payGroup,
  onOpenChange,
  onSuccess,
}: Omit<EditPayGroupModalProps, 'open'>) {
  const frequency = payGroup.pay_frequency ?? 'MONTHLY'
  const payDayType = payGroup.pay_day_type ?? 'FIXED_DAY'
  const weeklyDefault = payGroup.pay_day && payGroup.pay_day >= 1 && payGroup.pay_day <= 7
    ? String(payGroup.pay_day)
    : '1'
  ...
  const {
    ...
    defaultValues: {
      ...
      pay_day_of_month: payGroup.pay_day != null ? String(payGroup.pay_day) : '',
      pay_day_of_week: weeklyDefault,
      anchor_date: payGroup.anchor_date ?? '',
      currency: 'NGN',
  ...
  const selectedFrequency = watch('pay_frequency')
  const selectedPayDayType = watch('pay_day_type')

  React.useEffect(() => {
    if (selectedFrequency !== 'MONTHLY' && selectedPayDayType !== 'FIXED_DAY') {
      setValue('pay_day_type', 'FIXED_DAY', { shouldDirty: false, shouldValidate: false })
    }
  }, [selectedFrequency, selectedPayDayType, setValue])
  const showMonthlyFixedDay = selectedFrequency === 'MONTHLY' && selectedPayDayType === 'FIXED_DAY'
  const showWeekdaySelect = selectedFrequency === 'WEEKLY' || selectedFrequency === 'BI_WEEKLY'
  const showBiWeeklyStartDate = selectedFrequency === 'BI_WEEKLY'
  ...
  const onSubmit = async (values: FormValues) => {
    ...
    const result = await updatePayGroup(payGroup.id, {
      name: values.name,
  ...
  return (
    <>
      <DialogHeader>
        ...
        <DialogDescription>
          Update this pay group schedule and payroll settings.
        </DialogDescription>
      </DialogHeader>

      <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
        <div className="space-y-2">
          <Label htmlFor="edit-pay-group-name">Pay group name</Label>
          <Input
            id="edit-pay-group-name"
            {...register('name')}
        ...
        <div className="space-y-2">
          <Label htmlFor="edit-pay-frequency">Frequency</Label>
          <select
        ...
        {selectedFrequency === 'MONTHLY' ? (
          <div className="space-y-2">
            <Label htmlFor="edit-pay-day-type">Payment schedule</Label>
            <select
              id="edit-pay-day-type"
        ...
        {showMonthlyFixedDay ? (
          <div className="space-y-2">
            <Label htmlFor="edit-pay-day-of-month">Pay day (1 - 31)</Label>
            <Input
              id="edit-pay-day-of-month"
              type="number"
              min={1}
              max={31}
              placeholder="e.g. 25"
        ...
        {showWeekdaySelect ? (
          <div className="space-y-2">
            <Label htmlFor="edit-pay-day-of-week">Pay day of the week</Label>
            <select
              id="edit-pay-day-of-week"
        ...
        {showBiWeeklyStartDate ? (
          <div className="space-y-2">
            <Label htmlFor="edit-anchor-date">Start date</Label>
            <Input
              id="edit-anchor-date"
              type="date"
              min={todayIso()}
        ...
        <div className="flex items-center gap-2">
          <input
            id="edit-auto-generate-payroll"
            type="checkbox"
            {...register('auto_generate_payroll')}
            disabled
            className="size-4 rounded border-input"
          />
          <Label htmlFor="edit-auto-generate-payroll" className="font-normal cursor-not-allowed">
...
export function EditPayGroupModal({ payGroup, open, onOpenChange, onSuccess }: EditPayGroupModalProps) {
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-md">
        {open ? (
          <EditPayGroupFormBody
            key={payGroup.id}
            payGroup={payGroup}
