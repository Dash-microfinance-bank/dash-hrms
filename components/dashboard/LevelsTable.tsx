...
import { CreateLevelModal } from '@/components/dashboard/CreateLevelModal'
import { EditLevelModal } from '@/components/dashboard/EditLevelModal'
import { DeleteLevelConfirmModal } from '@/components/dashboard/DeleteLevelConfirmModal'

type LevelsTableProps = {
  data: EmployeeLevelRow[]
}

export function LevelsTable({ data }: LevelsTableProps) {
  const router = useRouter()
  const [createOpen, setCreateOpen] = useState(false)
  const [editingLevel, setEditingLevel] = useState<EmployeeLevelRow | null>(null)
  const [deletingLevel, setDeletingLevel] = useState<EmployeeLevelRow | null>(null)
  const [sorting, setSorting] = useState<SortingState>([{ id: 'created_at', desc: true }])
  const [globalFilter, setGlobalFilter] = useState('')

  const columns = useMemo<ColumnDef<EmployeeLevelRow>[]>(
    () => [
      {
        id: 'sn',
        header: 'S/N',
        ...
        cell: ({ row }) => (
          <DropdownMenu>
            ...
            <DropdownMenuContent align="end">
              <DropdownMenuItem onClick={() => setEditingLevel(row.original)}>
                Edit
              </DropdownMenuItem>
              <DropdownMenuItem
  ...
  return (
    <div className="space-y-4 bg-card py-3 px-3 rounded-md">
              ...
              <TableRow>
                <TableCell colSpan={4} className="h-32 text-center align-middle">
                  <p className="text-sm text-muted-foreground">
                    {isEmpty ? 'No levels found.' : 'No levels match your search.'}
                  </p>
                </TableCell>
              </TableRow>
            ) : (
      ...
      {rows.length > 0 && (
        <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
          <p className="text-xs text-muted-foreground">
            Showing {rows.length} of {data.length} levels
          </p>
      ...
      <CreateLevelModal open={createOpen} onOpenChange={setCreateOpen} onSuccess={handleSuccess} />
